...
architecture is designed to be as simple as possible but provide a sensible target
for the compilation of complex programs using simple strategies. The
implementation of the processor and its supporting tooling is small and self-contained
so as to be understandable and easily extendable. Besides being an
...
First, some background. The project is based on the Hex processor
...
and A, B and C registers for expression evaluation. These kinds of architectural
...
language](https://en.wikipedia.org/wiki/Occam_(programming_language)) but is
...
a multiple of a byte. In the included implementation the word size is 4 bytes.
...
<tr>
  <td>&nbsp;&nbsp;<code>SUB</code></td>
  <td><code>areg := areg - breg</code></td>
  <td>Subtract breg from areg and set areg to the result</td>
</tr>
<tr>
  <td>&nbsp;&nbsp;<code>SVC</code></td>
  <td>-</td>
...
Negative values always require a negative prefix to fill the topmost ``oreg``
...
prefixing and the corresponding overhead to form larger immediates. The following
...
control to the system to complete an action such as reading from or writing to a
...
variants of load/store instructions are influenced by their targeting from a
...
following program implements bubble sort to sort an array of four elements:
...
- The scoping and abbreviation rules are defined so that formal parameters
  act as abbreviations of the supplied actuals. This allows processes and
  functions to be compiled either as closed subroutines or by substitution of
...
increases with the memory capacity, an implementation of Hex accessing a large
...
Taking the most basic example of an assembly program that simply exits:
...
a similarly basic program written in X:
...
And, when simulated, it produces the following trace where execution through
...
A fuller example is 'Hello World', where the main process is simply:
...
inspecting the trace makes it clear that most time is spent in the arithmetic
routines.
...
Before emission, the values of labels must be determined, which involves solving two
...
The compiler works in a traditional way, structured as a sequence of phases, first
...
simplifications. First, the small set of features in X makes it straightforward
to map directly to machine instructions rather than an intermediate
...
- [Experiments in Computer System
  ...
  is a walkthrough of an implementation of a Tiny Register Machine (TRM) in
...
- [Nibbler 4 Bit CPU](https://www.bigmessowires.com/nibbler/) is a complete
  description of a processor that has been implemented in discrete logic chips.

- [Discrete logic IC CPU](https://imihajlov.tk/blog/posts/ccpu/) is an
