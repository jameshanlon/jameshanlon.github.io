...
def text_object(use_count=True):
    if use_count:
        ...
        return "{}{}{}".format(
            count, random.choice(MODIFIERS), random.choice(TEXT_OBJECTS)
        )
    else:
...
"""
col1 = "<br>".join(
    ["<code>{}</code>".format(cgi.escape(motion())) for x in range(REPEAT)]
)
col2 = "<br>".join(
    ["<code>{}</code>".format(cgi.escape(motion())) for x in range(REPEAT)]
)
col3 = "<br>".join(
    [
        "<code>{}</code>".format(cgi.escape(text_object(use_count=False)))
...
col5 = "<br>".join(
    [
        "<code>{}</code>".format(cgi.escape(operator_text_object()))
        for x in range(REPEAT)
    ]
)
col6 = "<br>".join(
...
col7 = "<br>".join(
    ["<code>{}</code>".format(cgi.escape(operator_motion())) for x in range(REPEAT)]
)
col8 = "<br>".join(
    ["<code>{}</code>".format(cgi.escape(operator_motion())) for x in range(REPEAT)]
)
